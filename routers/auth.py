...
async def line_callback(code: str, state: str = "", db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 只有當 user 有設定 camera_email 時才換 camera token
        # LINE 帳號沒有 email 授權，不能用 oauth.local email 打 Camera Backend
        camera_tokens = {}
        if user and user.camera_email:
            ...
            print(f"[LINE callback] skipping camera token (no camera_email set for user {user.id if user else 'unknown'})")
